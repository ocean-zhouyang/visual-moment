package com.sjsk.passjava.utils;
...
public class JwtTokenUtil {
    ...
    private void cacheToken(String userId, Map<String, Object> tokenMap) {
        redissonClient.getMap(JWT_CACHE_KEY + userId).put(ACCESS_TOKEN, tokenMap.get(ACCESS_TOKEN));
        redissonClient.getMap(JWT_CACHE_KEY + userId).put(REFRESH_TOKEN, tokenMap.get(REFRESH_TOKEN));
        redissonClient.getMap(userId).expire(jwtProperties.getExpiration() * 2, TimeUnit.MILLISECONDS);
    }
    ...
    public Map<String, Object> refreshTokenAndGenerateToken(String userId, String username) {
        Map<String, Object> tokenMap = buildToken(userId, username);
        redissonClient.getMap(JWT_CACHE_KEY + userId).delete();
        cacheToken(userId, tokenMap);
    ...
    public boolean removeToken(String userId) {
        return Boolean.TRUE.equals(redissonClient.getMap(JWT_CACHE_KEY + userId).delete());
    }
    ...
    public Boolean isRefreshTokenNotExistCache(String token) {
        ...
        String refreshToken = (String) redissonClient.getMap(JWT_CACHE_KEY + userId).get(REFRESH_TOKEN);
